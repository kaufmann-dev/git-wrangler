...
func Generate(ctx context.Context, repos []Repository, cfg Config, out io.Writer, confirm func(string) bool) (*Plan, error) {
	...
	return buildPlan(items, results, stats, cfg.WorkDir, cfg.RemoveCoauthors)
}
...
func collectRepoItems(ctx context.Context, repoIndex int, repo Repository, gitClient git.Client, skipConventional, requireScope bool, progress func(ProgressEvent)) ([]item, Stats, error) {
	...
	for commitIndex, commit := range commits {
		...
		if commit.isMerge || IsAutoGeneratedMessage(commit.message) {
			stats.SkippedAutoGenerated++
			continue
		}
...
func IsAutoGeneratedMessage(message string) bool {
...
func autoGeneratedSubject(message string) bool {
	return IsAutoGeneratedMessage(message)
}
...
func buildPlan(items []item, results map[string]Message, stats Stats, workDir string, removeCoauthors bool) (*Plan, error) {
	type key struct {
		index  int
		dir    string
		name   string
	...
	for _, item := range items {
		...
		message := trailers.MergeGenerated(item.OldMessage, FormatMessage(result), removeCoauthors)
