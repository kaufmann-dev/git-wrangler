...
func TestBuildPlanPreservesTrailersAndCanRemoveOnlyCoauthors(t *testing.T) {
	items := []item{{
		ID:         "c000001",
		RepoIndex:  0,
		RepoDir:    "repo",
		...
		RepoGitDir: "repo/.git",
		Hash:       "abc123",
		OldMessage: "old subject\n\nold body\n\nReviewed-BY: R <r@example.test>\nco-AUTHORED-by: C <c@example.test>\nToken: first\n folded",
	}}
	results := map[string]Message{"c000001": {Subject: "feat(cli): new subject", Body: "New body."}}
	for _, tc := range []struct {
		...
		},
		{
			...
			removeCoauthors: true,
			want:            []string{`Reviewed-BY: R <r@example.test>`, `Token: first\n folded`},
			unwanted:        `co-AUTHORED-by`,
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			plan, err := buildPlan(items, results, Stats{RepoCount: 1, TotalCommits: 1}, t.TempDir(), tc.removeCoauthors)
			if err != nil {
				t.Fatal(err)
			}
			if len(plan.Repos) != 1 || plan.Repos[0].ChangedCount != 1 {
				t.Fatalf("plan = %#v", plan)
			}
			callback, err := os.ReadFile(plan.Repos[0].CallbackFile)
			if err != nil {
				t.Fatal(err)
			}
			...
			if tc.unwanted != "" && strings.Contains(text, tc.unwanted) {
				t.Fatalf("callback contains %q:\n%s", tc.unwanted, text)
			}
		})
	}
}
