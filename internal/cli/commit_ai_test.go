...
func TestCommitAIStagesSkipsCommitsAndReportsSummary(t *testing.T) {
	...
	runner := fakeRunner{
		...
		run: func(ctx context.Context, dir string, env []string, name string, args ...string) (string, string, error) {
			...
			case joined == "add -A":
				return "", "", nil
			case repoName == "clean" && joined == "diff --cached --quiet":
				return "", "", nil
			case repoName == "dirty" && joined == "diff --cached --quiet":
				return "", "", errors.New("dirty")
			case repoName == "dirty" && joined == "diff --cached --name-status":
				return "M\tfile.txt\n", "", nil
	...
	runner := fakeRunner{
		...
		run: func(ctx context.Context, dir string, env []string, name string, args ...string) (string, string, error) {
			...
			case joined == "diff --cached --quiet":
				return "", "", errors.New("dirty")
			case joined == "diff --cached --name-status":
				return "M\tfile.txt\n", "", nil
