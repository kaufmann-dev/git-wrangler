...
Clones GitHub repositories based on specified criteria (visibility, user, limit) and organizes them into a designated directory. Checks for existing repositories and skips them automatically. Requires `gh` for GitHub transport.
...
- For `private` or `all` visibility, Git Wrangler GitHub auth must be configured with `git-wrangler init` or `git-wrangler config set github.auth`
