...
- Injected TTY eligibility plus cursor editing, buffered multi-prompt input, hidden input, required, guided, summary, and confirmation helper behavior.
...
- Cancellation restoring terminal state for visible and secret input, suppressing later prompts and summaries, preventing mutation, and rendering one cancellation status.
