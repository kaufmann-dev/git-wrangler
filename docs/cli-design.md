...
Refresh `origin` first unless `--no-fetch` is set. Fetch failures stop before scanning, planning, preview, prompt, or mutation. `--no-fetch` prints a warning before scanning. Preparation progress completes before durable output. Rewrite preview starts with a global date-plan header containing repository count, selected commit count, target range, planning timezone, active days, rest days, forced-active days, final post-topology median/p90/maximum commits per active day, filters, frequency, spread, time window or weekday schedule, and seed source. Do not show fixed-threshold workload concepts such as `5+/day`. Candidate repo blocks show selected commits, planned range, timezone, compact old-to-new samples, and tag/signature warnings when detected. Warn and prompt once. Apply with progress and summarize `rewritten`, `skipped`, and `failed`.

### `rewrite-hours`

Refresh `origin` first unless `--no-fetch` is set. Fetch failures stop before scanning, planning, preview, prompt, or mutation. `--no-fetch` prints a warning before scanning. Preparation progress completes before durable output. Preview starts with an hour-plan header containing repository count, selected commit count, time window or weekday schedule, and planning timezone. Candidate repo blocks show only commits scheduled for rewriting, planned range, timezone, compact old-to-new samples, and tag/signature warnings when detected. Repositories with only unscheduled commits are skipped before prompting. Warn and prompt once when at least one repository has scheduled rewrites. Apply with progress and summarize `rewritten`, `skipped`, and `failed`.
