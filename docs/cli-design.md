...
Refresh `origin` first unless `--no-fetch` is set. Fetch failures stop before the destructive notice, prompt, or mutation. `--no-fetch` prints a warning before continuing. `--rewrite-after` and `--rewrite-before` filter by current author date before preview and baseline capture. Print a concise destructive notice with repository count, new identity, and current author date filter. Warn and prompt once. Apply with progress. Suppress per-repo success lines, print origin-restore warnings and failures, and summarize.
...
Refresh `origin` first unless `--no-fetch` is set. Fetch failures stop before scanning, planning, preview, prompt, or mutation. `--no-fetch` prints a warning before scanning. Preparation progress completes before durable output. `--rewrite-after` and `--rewrite-before` filter by current author date, and omitted target bounds are inferred from selected current author dates. Rewrite preview starts with a global date-plan header containing repository count, selected commit count, target range, planning timezone, active days, rest days, forced-active days, final post-topology median/p90/maximum commits per active day, filters, frequency, spread, time window or day-of-week schedule, and seed source. Do not show fixed-threshold workload concepts such as `5+/day`. Candidate repo blocks show selected commits, planned range, timezone, compact current-to-new samples, and tag/signature warnings when detected. Warn and prompt once. Apply with progress and summarize `rewritten`, `skipped`, and `failed`.

### `rewrite-hours`

Refresh `origin` first unless `--no-fetch` is set. Fetch failures stop before scanning, planning, preview, prompt, or mutation. `--no-fetch` prints a warning before scanning. Preparation progress completes before durable output. `--rewrite-after` and `--rewrite-before` filter by current author date before day-of-week schedule filtering. Preview starts with an hour-plan header containing repository count, selected commit count, time window or day-of-week schedule, and planning timezone. Candidate repo blocks show only commits scheduled for rewriting, planned range, timezone, compact current-to-new samples, and tag/signature warnings when detected. Repositories with only unscheduled commits are skipped before prompting. Warn and prompt once when at least one repository has scheduled rewrites. Apply with progress and summarize `rewritten`, `skipped`, and `failed`.
...
`config show` uses non-secret key/value sections. Secret values are never printed. `config set` and `config unset` use standard `OK Updated <key>` and `OK Unset <key>` lines. `config file remove-secrets show` validates before printing the effective paths, labeling them as built-in defaults when no file exists yet, and `config file remove-secrets edit` validates after the editor exits. Errors should say whether a key is unknown, a value is missing, plaintext secrets are not accepted, or a file-backed config is invalid.
