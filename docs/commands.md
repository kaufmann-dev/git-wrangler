...
| `rewrite-hours`     | History | Discovered repos or exact `--repo`           | Moves commit timestamps into one daily window through `git-filter-repo`                       | Prompts once for all candidates unless `--yes`; declined rewrites are skips                             | `git`, `git-filter-repo` or `git filter-repo` | None                                                      |
...
| `--window`                                    | `rewrite-dates`, `rewrite-hours`                                                                                                                                                                              | Uses one same-day `HH:MM-HH:MM` time window. For `rewrite-dates`, omitting it preserves generated per-day working hours; for `rewrite-hours`, it is required.                                       |
...
`--window HH:MM-HH:MM` keeps the existing calendar, active/rest day, quota, seed, frequency, spread, and topology planning, but replaces generated per-day working hours with the supplied same-day window. The same explicit window is applied to every targeted repository. Omitting `--window` preserves the generated working-hour behavior. Date redistribution and explicit windowing happen in one `git-filter-repo` pass.
...
`--window HH:MM-HH:MM` is required and must be a same-day window with the start before the end. The command considers commits reachable from the same local branch ref surface used by `rewrite-dates`, preserves each commit's current calendar date in the run planning timezone, and rewrites both author and committer dates into that daily window. Timestamps are deterministic, sorted, and second-resolution; duplicate seconds are allowed only when a day's commit count exceeds the number of seconds in the window.

The explicit window is applied uniformly to every targeted repository. Parent/child timestamp ordering is validated before mutation. The command creates or extends the same shared baseline as `rewrite-authors`, `rewrite-commits`, and `rewrite-dates`, then applies one `git-filter-repo` date callback per repository and updates current SHA mappings from `commit-map`.
