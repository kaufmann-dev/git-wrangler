...
The planner seed comes from `--seed`, then existing per-repository rewrite state, then a generated seed. Preview output shows the seed source. The command stores per-repository state in a JSON blob ref at `refs/git-wrangler/state/rewrite-dates`, creates backup refs below `refs/git-wrangler/backup/rewrite-dates/<run-id>/...`, records branch heads and their backup refs for rollback, reads `.git/filter-repo/commit-map` after each normal rewrite, and updates only current SHA mappings. Stored original dates are never overwritten.
