...
Commands with `--repo`: `activity`, `pull`, `fetch`, `push`, `commit`, `fix-gitignore`, `license`, `rename-branch`, `reset`, `review`, `untrack`, `remove-secrets`, `rewrite-authors`, `rewrite-commits`, `rewrite-dates`, `info`, `status`, and `rename-repo`.
...
| `rewrite-dates`   | History | Discovered repos or exact `--repo`           | Normal rewrite through `git-filter-repo`; exact rollback through stored backup refs           | Prompts once for all candidates unless `--yes`; declined rewrites are skips                             | `git`; `git-filter-repo` or `git filter-repo` for normal rewrites | None                                                      |
...
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--repo`                                          | `activity`, `pull`, `fetch`, `push`, `commit`, `fix-gitignore`, `license`, `rename-branch`, `reset`, `review`, `untrack`, `remove-secrets`, `rewrite-*`, `info`, `status`, `rename-repo` | Targets exactly one working-tree repository. Does not discover nested repositories.                                                                                                                 |
...
| `--seed`, `--frequency`, `--spread`, `--rollback` | `rewrite-dates`                                                                                                                                                                          | Controls deterministic date planning, selects `low`, `medium`, or `high` activity frequency and quota spread, or restores known history from stored rollback state.                                 |
...
| Behavior                                         | Commands                                                                                                                                                                                       |
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Read-only repository scans with up to 32 workers | `activity`, `status`, `review`, `info`, scan phases of `fix-gitignore`, `remove-secrets`, `rewrite-commits`, `rewrite-dates`                                                                   |
...
The planner seed comes from `--seed`, then existing per-repository rewrite state, then a generated seed. Preview output shows the seed source, planning timezone, active days, rest days, forced-active days, selected commit count, frequency, spread, and final post-topology median, p90, and maximum commits per active day. The command stores versioned per-repository state in a JSON blob ref at `refs/git-wrangler/state/rewrite-dates`. Unsupported or unversioned rewrite-date state fails before mutation. For each branch, the first rewrite stores the original head and original backup ref; later rewrites keep that baseline and update only the current rewritten baseline head. The command reads `.git/filter-repo/commit-map` after each normal rewrite and updates only current SHA mappings. Stored original dates are never overwritten.

`--rollback` restores the first stored baseline, not the most recent rewrite layer. Branches already at or descended from the original head are skipped. Branches exactly at the current rewritten baseline head are moved back to the original head, preserving original hashes and signatures for known history. Branches with new commits on top of the current rewritten baseline restore the original base and replay only those new commits with `git commit-tree`, preserving their tree, message, authorship, and stored original dates where possible; replayed commits may receive new hashes or lose signatures. Rollback requires direct branch rollback metadata and a clean working tree, creates rollback backup refs before moving branch refs, then resets the checked-out branch worktree/index to the new `HEAD`. A branch with no direct metadata is skipped only when it contains no known rewritten commits; otherwise it fails before mutation. Rollback uses the same fetch, preview, destructive warning, and aggregate prompt behavior as normal rewrites, and summarizes `rolled back`, `skipped`, and `failed`.
