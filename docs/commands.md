...
| `--seed`, `--intensity`, `--rollback`         | `rewrite-dates`                                                                                                                                                              | Controls deterministic date planning, selects `low`, `medium`, or `high` activity density, or restores known commits to stored original dates.                                                      |
...
The planner seed comes from `--seed`, then existing per-repository rewrite state, then a generated seed. Preview output shows the seed source. The command stores per-repository state in a JSON blob ref at `refs/git-wrangler/state/rewrite-dates`, creates backup refs below `refs/git-wrangler/backup/rewrite-dates/<run-id>/...`, reads `.git/filter-repo/commit-map` after each rewrite, and updates only current SHA mappings. Stored original dates are never overwritten.

`--rollback` uses the stored state to restore known current commits to their original author and committer dates. New commits that are not known in state keep their current dates, though their hashes may still change if parent commits are rewritten. Rollback uses the same fetch, preview, warning, aggregate prompt, origin-restore, and final `rewritten`, `skipped`, `failed` summary behavior as normal rewrites.
