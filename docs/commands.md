...
Commands with `--repo`: `activity`, `pull`, `fetch`, `push`, `commit`, `fix-gitignore`, `license`, `rename-branch`, `reset`, `review`, `untrack`, `remove-secrets`, `rewrite-authors`, `rewrite-commits`, `rewrite-dates`, `rewrite-hours`, `rollback-rewrites`, `info`, `status`, and `rename-repo`.
...
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--repo`                                      | `activity`, `pull`, `fetch`, `push`, `commit`, `fix-gitignore`, `license`, `rename-branch`, `reset`, `review`, `untrack`, `remove-secrets`, `rewrite-*`, `rollback-rewrites`, `info`, `status`, `rename-repo` | Targets exactly one working-tree repository. Does not discover nested repositories.                                                                                                                                                                           |
...
| `--start-date`, `--end-date`                  | `rewrite-dates`                                                                                                                                                                                               | Optional `YYYY-MM-DD` target-range bounds. Missing bounds are inferred globally from selected original author dates.                                                                                                                                          |
| `--rewrite-before`, `--rewrite-after`         | `rewrite-dates`                                                                                                                                                                                               | Optional commit-selection bounds using stored original author dates. Together they select `after <= commit < before`.                                                                                                                                         |
...
| `--window`                                    | `rewrite-dates`, `rewrite-hours`                                                                                                                                                                              | Uses one same-day `HH:MM-HH:MM` window for every day, or a weekday schedule such as `mon-fri=16:00-20:00,sat=10:00-14:00`. For `rewrite-dates`, omitting it preserves generated per-day working hours; for `rewrite-hours`, a non-empty schedule is required. |
...
| Behavior                                         | Commands                                                                                                                                                                                                        |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Read-only repository scans with up to 32 workers | `activity`, `status`, `review`, `info`, scan phases of `fix-gitignore`, `remove-secrets`, `rewrite-commits`, `rewrite-dates`, `rewrite-hours`, `rollback-rewrites`                                              |
...
Before mutation, it adds missing entries for commits reachable from local branch refs to the shared rewrite baseline under `.git/git-wrangler/baseline/`. Existing baseline entries keep their first-baselined metadata. After a successful `git-filter-repo` run, only current SHA mappings are updated from `.git/filter-repo/commit-map`.

### `rewrite-commits`

`rewrite-commits` validates the configured AI endpoint, model, and credentials before checking dependencies or refreshing `origin`, then scans repositories. Any fetch failure stops before scanning, AI requests, preview, confirmation, or mutation. With `--no-fetch`, it warns that local remote-tracking refs may miss remote-only commits before scanning and before the normal AI data-send confirmation path. Generation may prompt before sending data; applying generated messages is a separate all-repository confirmation. Old commit messages are not sent as model context. The AI context includes file paths, stats, a compact change summary, and redacted diff snippets; repository metadata, summaries, file lists, and stats are preserved ahead of diff text when internal context bounds truncate input. `--batch-size` is a maximum commits-per-request value; dynamic packing may send smaller batches to stay within bounded request context, and provider context-limit errors are retried with smaller internal request budgets. For better high-level messages on broad changes, use `--body`, reduce `--batch-size`, or choose a stronger configured model. Declining either confirmation is a successful no-op before mutation. Final apply output is aggregate: rewritten commit messages, updated repositories, and failures.
...
`--window` is required and must be either one same-day `HH:MM-HH:MM` window with the start before the end, or a non-empty comma-separated weekday schedule such as `mon-fri=16:00-20:00,sat=10:00-14:00`. The command considers commits reachable from the same local branch ref surface used by `rewrite-dates`, preserves each commit's current calendar date in the run planning timezone, and rewrites both author and committer dates into the scheduled window for that weekday. Timestamps are deterministic, sorted, and second-resolution; duplicate seconds are allowed only when a day's commit count exceeds the number of seconds in the window.

Plain `--window HH:MM-HH:MM` is applied uniformly to every targeted repository and every weekday. Schedule entries apply only to their listed weekdays; commits on omitted weekdays keep their existing timestamps and are excluded from the date callback and baseline capture. Parent/child timestamp ordering is validated before mutation. The command creates or extends the same shared baseline as `rewrite-authors`, `rewrite-commits`, and `rewrite-dates`, then applies one `git-filter-repo` date callback per repository and updates current SHA mappings from `commit-map`.
