...
| `--seed`, `--intensity`, `--rollback`         | `rewrite-dates`                                                                                                                                                                          | Controls deterministic date planning, selects `low`, `medium`, or `high` activity density, or restores known history from stored rollback state.                                                    |
...
`--intensity` controls the generated calendar shape without changing the command surface. Active-day density scales dynamically from selected commits, available non-rest days, and intensity; there is no fixed daily capacity or maximum. `low` creates sparse work sessions with long gaps and lower workload variance. `medium` is the clustered weekday-heavy default with multi-day gaps and moderate variance. `high` creates denser sessions with shorter gaps, stronger multi-day bursts, and higher variance. When a plan has multiple sessions, they are distributed broadly across the complete target range without guaranteeing activity on the exact start or end dates. All intensities reduce weekend workload and preserve inactive and rest periods when topology permits.
